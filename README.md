...
- 100% Customized
- 100% Free
- **16 built-in plugins** for dynamic variables: GitHub stars / user stats, npm / PyPI / RubyGems / crates.io / Docker Hub downloads, RSS latest post, YouTube latest video, follower counts (Bluesky / Mastodon / Dev.to), Hacker News karma, Stack Overflow reputation, Reddit karma, and a Countdown helper. Add your own with one Ruby file — or have Claude Code scaffold one for you ([guide](#plugins))
...
|---|---|---|---|
| `GithubRepoStarsCountPlugin` | Star count for one or more GitHub repos | <pre>- GithubRepoStarsCountPlugin:<br>    - owner/repo</pre> | `{{vars.GithubRepoStarsCountPlugin['owner/repo']}}` |
| `NpmPackageDownloadsPlugin` | Last-week downloads from api.npmjs.org | <pre>- NpmPackageDownloadsPlugin:<br>    - lodash</pre> | `{{vars.NpmPackageDownloadsPlugin['lodash']}}` |
| `RssFeedLatestPostPlugin` | Title + URL of the latest post in any RSS or Atom feed | <pre>- RssFeedLatestPostPlugin:<br>    - blog: "https://blog.example.com/feed"</pre> | `{{vars.RssFeedLatestPostPlugin.blog.title}}` / `.link` / `.date` |
| `BlueskyFollowersCountPlugin` | Bluesky follower count via public XRPC | <pre>- BlueskyFollowersCountPlugin:<br>    - me.bsky.social</pre> | `{{vars.BlueskyFollowersCountPlugin['me.bsky.social']}}` |
...
| `YouTubeChannelLatestVideoPlugin` | Latest video from a YouTube channel (Atom feed) | <pre>- YouTubeChannelLatestVideoPlugin:<br>    - main: "UCXuqSBlHAE6Xw-yeJA0Tunw"</pre> | `{{vars.YouTubeChannelLatestVideoPlugin.main.title}}` / `.link` |
| `GitHubUserStatsPlugin` | Followers / following / public_repos for a GitHub user | <pre>- GitHubUserStatsPlugin:<br>    - octocat</pre> | `{{vars.GitHubUserStatsPlugin['octocat'].followers}}` |
| `PyPiPackageDownloadsPlugin` | Last-week PyPI downloads via pypistats.org | <pre>- PyPiPackageDownloadsPlugin:<br>    - requests</pre> | `{{vars.PyPiPackageDownloadsPlugin['requests']}}` |
...
| `DockerHubPullsPlugin` | Pull count for a Docker Hub image (use `library/<name>` for official) | <pre>- DockerHubPullsPlugin:<br>    - library/nginx</pre> | `{{vars.DockerHubPullsPlugin['library/nginx']}}` |
...
All network plugins are fail-safe: if the request fails, they return `0` (or an empty struct for the RSS plugin) so the build never crashes. Set sensible cron in [Automatic Redeployment](#automatic-redeployment) to keep values fresh.
